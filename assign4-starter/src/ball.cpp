...
Ball::Ball(int id, GWindow* w) {
    window = w;
    this->id = id;
    x = randomInteger(0, window->getWidth());
    y = randomInteger(0, window->getHeight());
...
void Ball::move() {
    ...
    if (x < 0 || x + SIZE > window->getWidth()) {
        if (x < 0) {x = 0;}
        if (x + SIZE > window->getWidth()) {x = window->getWidth() - SIZE;}
        vx = -vx;
    }
    // if outside top or bottom edge, bounce vertically
    if (y < 0 || y + SIZE > window->getHeight()) {
        if (y < 0) {y = 0;}
        if (y + SIZE > window->getHeight()) {y = window->getHeight() - SIZE;}
        vy = -vy;
    }
}
