...
void PQSortedArray::enqueue(DataPoint elem) {
    ...
    for (int i = size() - 1; i > 0; i--) {
        if (elements[i-1].priority <= elem.priority) {
            elements[i] = elements[i - 1];
            elements[i-1] = elem;
        } else if (elements[i-1].priority > elem.priority){
            elements[i] = elem;
            break;
        }
    }
...
STUDENT_TEST("Student Tests: Small data set") {
    PQSortedArray pq;
    DataPoint trip = {"Trip", 106};
    DataPoint kylie = {"Kylie", 10};
    DataPoint nick = {"Nick", -3};
    DataPoint julie = {"Julie", 50};
    DataPoint zhuli = {"Zhu Li", 50};
    pq.enqueue(trip);
    pq.enqueue(kylie);
    pq.enqueue(nick);
    pq.enqueue(julie);
...
STUDENT_TEST("Operational time") {
    PQSortedArray pq;
    TIME_OPERATION(1000, fillQueue(pq, 1000));
    TIME_OPERATION(1000, emptyQueue(pq, 1000));
    TIME_OPERATION(1000, fillQueue(pq, 5000));
    TIME_OPERATION(1000, emptyQueue(pq, 5000));
    TIME_OPERATION(10000, fillQueue(pq, 10000));
    TIME_OPERATION(10000, emptyQueue(pq, 10000));
