 ...
 */
const string kYourName = "GIANGNGUYEN";

/* Change this constant to the path out of your labyrinth. */
const string kPathOutOfNormalMaze = "WSESSNNENEWSSSE";
...
bool isPathToFreedom(MazeCell* start, string moves, Set<string> needs) {
    MazeCell* trace = start;
    Set<string> collected;
    if (start->contents.size() > 0) {
        collected.add(start->contents);
    }
    for (char step : moves) {
        if (step == 'N') {
            if (trace->north == nullptr) {
                return false;
            }
            trace = trace->north;
        } else if (step == 'W') {
            if (trace->west == nullptr) {
                return false;
            }
            trace = trace->west;
        } else if (step == 'E') {
            if (trace->east == nullptr) {
                return false;
            }
            trace = trace->east;
        } else if (step == 'S') {
            if (trace->south == nullptr) {
                return false;
            }
            trace = trace -> south;
        } else {
        ...
        if (trace->contents.size() > 0) {
            collected.add(trace->contents);
        }
        if (collected.containsAll(needs)) {return true;}
    }
    return false;
}
